...
pub trait Tuner: std::fmt::Debug {
    ...
    fn set_gain(&mut self, handle: &RtlSdrDeviceHandle, gain: TunerGain) -> Result<()>;
    fn set_freq(&mut self, handle: &RtlSdrDeviceHandle, freq: u32) -> Result<()>;
    fn set_bandwidth(&mut self, handle: &RtlSdrDeviceHandle, bw: u32, rate: u32) -> Result<()>;
...
impl Tuner for NoTuner {
    fn init(&mut self, handle: &RtlSdrDeviceHandle) -> Result<()> {Ok(())}
    ...
    fn read_gain(&self, handle: &RtlSdrDeviceHandle) -> Result<i32> { Ok(0) }
    fn set_gain(&mut self, handle: &RtlSdrDeviceHandle, gain: TunerGain) -> Result<()> {Ok(())}
    fn set_freq(&mut self, handle: &RtlSdrDeviceHandle, freq: u32) -> Result<()> {Ok(())}
    fn set_bandwidth(&mut self, handle: &RtlSdrDeviceHandle, bw: u32, rate: u32) -> Result<()> {Ok(())}
    fn get_xtal_freq(&self) -> Result<u32> { Ok(0) }
    fn set_xtal_freq(&mut self, freq: u32) -> Result<()> {Ok(())}
    fn get_if_freq(&self) -> Result<u32> { Ok(0) }
    fn exit(&mut self, handle: &RtlSdrDeviceHandle) -> Result<()> {Ok(())}
}