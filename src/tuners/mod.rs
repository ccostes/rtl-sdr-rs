...
impl Tuner for NoTuner {
    fn init(&mut self, handle: &Device) -> Result<()> {Ok(())}
    ...
    fn read_gain(&self, handle: &Device) -> Result<i32> { Ok(0) }
    fn set_gain(&mut self, handle: &Device, gain: TunerGain) -> Result<()> {Ok(())}
    fn set_freq(&mut self, handle: &Device, freq: u32) -> Result<()> {Ok(())}
    fn set_bandwidth(&mut self, handle: &Device, bw: u32, rate: u32) -> Result<()> {Ok(())}
    fn get_xtal_freq(&self) -> Result<u32> { Ok(0) }
    fn set_xtal_freq(&mut self, freq: u32) -> Result<()> {Ok(())}
    fn get_if_freq(&self) -> Result<u32> { Ok(0) }
    fn exit(&mut self, handle: &Device) -> Result<()> {Ok(())}
}