...
fn test_write_reg_u8() {
    ...
    let mut mock_handle = MockDeviceHandle::new();
    mock_handle.expect_write_control()
        .times(1)
        .with(eq(CTRL_OUT), eq(0), eq(addr), eq(index_expected), predicate::always(), eq(CTRL_TIMEOUT))
        .returning(move |_, _, _, _, data, _| {
            assert!(data.len() == 1);
...
fn test_demod_read_reg() {
    ...
    mock_handle.expect_read_control()
        .times(1)
        .with(eq(CTRL_IN), eq(0), eq((addr << 8) | 0x20), eq(page), predicate::always(), eq(CTRL_TIMEOUT))
        .returning(move |_, _, _, _, data, _| {
            data[0] = value;
            Ok(2)
        });
